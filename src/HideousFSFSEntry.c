                                                     ...
                                                     int *found)
{
    ...
    *found = 0;
    ...
    while (offset != -1) {
        ...
        extension_index = hideousfs_host_leaf_extension_index(image, temp_buffer, NULL);
...
static void read_image_config(HideousFS_Image *image)
{
    ...
    hideousfs_initialise_default_config(image);
    ...
    if (!hideousfs_parse_config(image, temp_buffer)) {
        hideousfs_initialise_default_config(image);
    }
}
...
static _kernel_oserror *read_image_path(HideousFS_Image *image)
{
    ...
    if (!hideousfs_copy_string(image->image_leaf, sizeof(image->image_leaf), last_dot + 1)) {
        return (_kernel_oserror *)&err_path_too_long;
    }
...
_kernel_oserror *hideousfs_fsentry_open_handler(_kernel_swi_regs *regs, void *private_word)
{
    ...
    if (image == NULL || !image->in_use || regs->r[1] == 0 ||
        hideousfs_is_active_image_path(image, (const char *)regs->r[1])) {
        return (_kernel_oserror *)&err_not_found;
    }
    ...
    if (!hideousfs_resolve_directory_path(image, (const char *)regs->r[1],
    ...
    projected_object = hideousfs_is_projected_object_name(image, (const char *)regs->r[1]);
    if (!hideousfs_build_backing_object_path(image, (const char *)regs->r[1],
                                   path_buffer, sizeof(path_buffer))) {
        return (_kernel_oserror *)&err_path_too_long;
    }

    if (hideousfs_is_hidden_object_path(image, (const char *)regs->r[1])) {
        return (_kernel_oserror *)&err_not_found;
    }
    ...
    if (!hideousfs_copy_string(file->backing_path, sizeof(file->backing_path), path_buffer)) {
...
_kernel_oserror *hideousfs_fsentry_file_handler(_kernel_swi_regs *regs, void *private_word)
{
    ...
    case fsfile_Save:
    case fsfile_Create:
        if (!hideousfs_build_writable_object_path(image, (const char *)regs->r[1],
                                        path_buffer, sizeof(path_buffer))) {
            return (_kernel_oserror *)&err_not_found;
        }
        projected_object = hideousfs_is_projected_object_name(image,
                                                    (const char *)regs->r[1]);
    ...
    case fsfile_CreateDir:
        if (!hideousfs_build_writable_directory_path(image, (const char *)regs->r[1],
                                           path_buffer, sizeof(path_buffer))) {
            return (_kernel_oserror *)&err_not_found;
        }
    ...
    case fsfile_ReadInfo:
        if (hideousfs_is_active_image_path(image, (const char *)regs->r[1])) {
            return (_kernel_oserror *)&err_not_found;
        }
                                           ...
                                           int with_info)
{
    ...
    if (hideousfs_path_has_mapped_component(image, dir_name)) {
        return (_kernel_oserror *)&err_not_found;
    }

    if (!hideousfs_build_backing_object_path(image, dir_name,
                                   path_buffer, sizeof(path_buffer))) {
        return (_kernel_oserror *)&err_path_too_long;
    }
    ...
    while (backing_offset != -1 && !stop) {
        ...
        if (hideousfs_is_active_image_entry(image, dir_name, leaf)) {
            skip = 1;
        }

        extension_index = hideousfs_mapped_extension_index(image, leaf, strlen(leaf));
        if (!skip && extension_index >= 0) {
            if (source->type == object_directory) {
                ...
                while (nested_offset != -1 && !stop) {
                    ...
                    if (!hideousfs_append_leaf_slash_extension(projected_name,
        ...
        if (!skip && hideousfs_host_leaf_extension_index(image, leaf, NULL) >= 0) {
            skip = 1;
        }

        if (!skip) {
            if (!hideousfs_copy_string(projected_name, sizeof(projected_name), leaf)) {
                return (_kernel_oserror *)&err_path_too_long;
            }
            emit_projected_dir_entry(dest, &used, &output_count,
...
static _kernel_oserror *read_dir(_kernel_swi_regs *regs, int with_info)
{
    ...
    if (!hideousfs_resolve_directory_path(image, dir_name, path_buffer, sizeof(path_buffer),
                                synthetic_extension,
                                sizeof(synthetic_extension))) {
        return (_kernel_oserror *)&err_path_too_long;
    }
    ...
    while (backing_offset != -1) {
        ...
        if (synthetic_extension[0] == '\0' &&
            hideousfs_is_active_image_entry(image, dir_name, leaf)) {
            skip = 1;
        }

        if (!skip && synthetic_extension[0] == '\0' &&
            hideousfs_is_mapped_extension(image, leaf)) {
            skip = 1;
        }

        extension_index = hideousfs_host_leaf_extension_index(image, leaf, &base_end);
        if (!skip && synthetic_extension[0] != '\0') {
            if (extension_index < 0 ||
            ...
            if ((seen_extensions & (1u << extension_index)) != 0) {
                ...
                if (!hideousfs_copy_string(projected_name, sizeof(projected_name),
                                 image->reverse_extensions[extension_index])) {
                    return (_kernel_oserror *)&err_path_too_long;
                }
                                      ...
                                      const char *to_name)
{
    ...
    if (!hideousfs_build_writable_object_path(image, from_name,
                                    path_buffer, sizeof(path_buffer)) ||
        !hideousfs_build_writable_object_path(image, to_name,
                                    temp_buffer, sizeof(temp_buffer))) {
        return (_kernel_oserror *)&err_not_found;
    }

    if (hideousfs_is_projected_object_name(image, to_name)) {
